...
export async function POST(
  ...
  const type = ["BIOLOGICAL", "CHEMICAL", "PHYSICAL", "RADIOLOGICAL"].includes(body.type)
    ? body.type
    : "BIOLOGICAL";
  const description = typeof body.description === "string" && body.description.trim() ? body.description.trim() : "New hazard";

  const hazard = await db.hazard.create({
    data: { processStepId: step.id, type, description },
  });
